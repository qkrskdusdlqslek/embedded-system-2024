 ...
 void rentBook(int id) {
   ...
   if(book != NULL) {
     if(book -> isAvailable) {
        book -> isAvailable = 0;   // 대여 상태로 변경
        printf("도서 '%s'를 대여하였습니다.\n, book -> title);
     }
     else if{
       printf("이 도서는 현재 대여 중입니다.\n);
     }
     else {
       printf("해당 ID를 가진 도서가 없습니다.\n);
     }
   }
 }
 */
 ...
 {
    int choice;
 // do while 문 : 한번은 {}안의 내용을 실행하고 그 후 while 조건을 확인 반복
    do{
       printf(" 도서관리 프로그램\n");
       printf("1. 도서추가\n");
       printf("2. 도서삭제\n");
